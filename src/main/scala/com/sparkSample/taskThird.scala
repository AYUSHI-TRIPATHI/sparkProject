...
object taskThird extends Serializable {
  ...
  def maxRatingMovies(Df3:DataFrame):Dataset[Row]= {
      Df3.select("rating").orderBy(desc("rating")).limit(1)
    //Df3.filter(col("rating")==="4.5").select("m_name","rating").show()
    //Df3.filter(col("rating")===max("rating")).count()
    //Df3.orderBy(desc("rating")).select("max(rating)","m_name").show()
  }
